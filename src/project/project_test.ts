...
Deno.test('handles an invalid entrypoint', async () => {
  await expect(getProjectFromEntrypoint(null)).rejects.toThrow('Project entry is invalid');
  await expect(getProjectFromEntrypoint(undefined)).rejects.toThrow('Project entry is invalid');
  await expect(getProjectFromEntrypoint({})).rejects.toThrow('Expected union value'); // TODO return better error message
});
